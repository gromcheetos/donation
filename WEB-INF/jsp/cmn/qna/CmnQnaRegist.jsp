...
function fn_egov_regist_qna(){
	...
	if (!validateQnaVO(document.qnaVO)) {	
		...
		if(confirm("<spring:message code="common.regist.msg" />")){	
			document.qnaVO.action = "<c:url value='/cmn/qna/insertQna.do'/>";
...
<!-- 전체 레이어 시작 -->
    ...
    <!-- 검색 필드 박스 시작 -->
   	<div id="search_field">
	<!-- 타이틀 -->
		<div id="search_field_loc"><h2>${pageTitle} <spring:message code="title.create" /></h2>
	</div>
	<form:form commandName="qnaVO" method="post" name="qnaVO" action="${pageContext.request.contextPath}/cmn/qna/insertQna.do"> 
	...
	<table class="wTable">
		...
		<tr>
			<th width="15%"><label for="qnaTitle">${title}<span class="pilsu">*</span></label></th>
			<td width="85%">
 						<form:input path="qnaTitle" title="${title} ${inputTxt}" size="70" maxlength="70" />
							<div><form:errors path="qnaTitle" cssClass="error" /></div>     
			</td>
		</tr>
		<!-- 질문내용 -->
		<c:set var="title"><spring:message code="comUssOlhQna.qnaVO.qnaContent"/> </c:set>
		<tr>
			<th width="15%"><label for="qnaContent">${title } <span class="pilsu">*</span></label></th>
			<td width="85%">
				<form:textarea path="qnaContent" title="${title} ${inputTxt}" class="textarea" cols="300" rows="20" />   
	...
	<ul>
		<li><a href="#LINK" class="s_submit" onclick="javascript:fn_egov_regist_qna(); return false;"><spring:message code="button.create" /></a></li>
		<%-- <input type="submit" class="s_submit" value="<spring:message code="button.create" />" /> --%>
		<li><a class="s_submit" href="<c:url value='/cmn/qna/selectQnaList.do' />" ><spring:message code="button.list" /></a></li>
	</ul>
