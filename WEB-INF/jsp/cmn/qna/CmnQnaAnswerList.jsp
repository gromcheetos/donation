...
function fn_egov_select_linkPage(pageNo){
	...
	document.qnaForm.action = "<c:url value='/cmn/qna/selectQnaAnswerList.do'/>";
...
function fn_egov_inquire_qnadetail(qnaSeq) {
	...
	document.qnaForm.qnaSeq.value = qnaSeq;
  	document.qnaForm.action = "<c:url value='/cmn/qna/selectQnaAnswerDetail.do'/>";
  	document.qnaForm.submit();
}
</script>
</head>
...
<form name="qnaForm" action="<c:url value='/cmn/qna/selectQnaAnswerList.do'/>" method="post" onSubmit="fn_egov_search_qna(); return false;"> 
<div class="board">
	...
	<tr>
		...
		<th><spring:message code="comUssOlhQna.qnaVO.qnaCk" /></th><!-- 진행상태 -->
	...
	<tr>
		...
		<td class="left"><a href="<c:url value='/cmn/qna/selectQnaAnswerDetail.do?qnaSeq=${resultInfo.qnaSeq}'/>" onClick="fn_egov_inquire_qnadetail('<c:out value="${resultInfo.qnaSeq}"/>');return false;"><c:out value='${fn:substring(resultInfo.qnaTitle, 0, 40)}'/></a></td>
		<td><c:out value='${resultInfo.qnaName}'/></td>
		<td><c:out value='${resultInfo.qnaCk}'/></td>
		<td><c:out value='${resultInfo.ansAd}'/></td>
	</tr>
	</c:forEach>
	</tbody>
	</table>
