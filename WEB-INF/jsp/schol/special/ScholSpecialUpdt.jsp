...
function fn_updateForm() {
	...
	if (!validateSchol(schol)) {	
		...
		if (confirm("<spring:message code="common.update.msg" />")) {
			schol.action = "<c:url value='/schol/special/updateArticle.do'/>";
			schol.submit();	
		}
...
function fn_egov_inqire_articlelist() {
	schol.action = "<c:url value='/schol/special/selectScholSpecialList.do'/>";
	schol.submit();
}
...
<div id="wrap">
 <!-- header 시작 -->
	...
	<!-- 상단타이틀 -->
	<div id="search_field">
	<div id="search_field_loc"><h2><spring:message code="scholspecial.list.specialUpdate" /></h2></div>
	</div>
	<form:form commandName="schol" name="schol" action="${pageContext.request.contextPath}/schol/special/updateArticle.do" method="post" onSubmit="fn_egov_updt_article(document.forms[0]); return false;">
	...
	<input type="hidden" name="scholSeq" value="<c:out value='${schol.scholSeq}'/>">
			...
			<div class="xbuttons" style="padding-top:10px;padding-bottom:10px;">
				<ul>
					<li><a href="#LINK" class="s_submit" onclick="javascript:fn_updateForm(); return false;"><spring:message code="button.update" /></a></li>
					<li><a class="s_submit" href="<c:url value='/schol/special/selectScholSpecialList.do' />?scholSeq=${schol.scholSeq}" ><spring:message code="button.list" /></a></li>
				</ul>
			</div>
